...
!
! !REVISION HISTORY:
!  SVN:$Id: vmix_kpp.F90 32223 2011-11-18 22:52:57Z njn01 $
   ...
   do k=1,km
    do iblock=1,nblocks_clinic
     do j=1,ny_block
      ...
      ! through the non convergence errors in the (LGM) marginal seas

      !----------------
      ! Caspian Sea
      !----------------

      if ( (tlatd .lt. 49.0_r8)  .and. (tlatd .gt. 40.0_r8)  .and.  &
           (tlond .gt. 22.0_r8) .and. (tlond .lt. 40.0_r8)) then
      ...
      else if ( (tlatd .lt. 52.0_r8)  .and. (tlatd .gt. 36.0_r8)  .and.  &
           (tlond .gt. 44.0_r8) .and. (tlond .lt. 52.0_r8)) then
           !bckgrnd_vdc(i,j,k,iblock) = bckgrnd_vdc1
           bckgrnd_vdc(i,j,k,iblock) = bckgrnd_vdc_eq
      ...
      else if ( (tlatd .lt. 30.0_r8)  .and. (tlatd .gt. 12.0_r8)  .and.  &
           (tlond .gt. 26.0_r8) .and. (tlond .lt. 44.0_r8)) then
           !bckgrnd_vdc(i,j,k,iblock) = bckgrnd_vdc1
           bckgrnd_vdc(i,j,k,iblock) = bckgrnd_vdc_eq
    ...
    do k=1,km
     ...
     do k=1,km
       where ((REGION_MASK(:,:,bid)) < c0 .and. (k<=KMT(:,:,bid)))
         KVIDEMIX(:,:,k,bid)=bckgrnd_vdc(:,:,k,bid)
       endwhere
     enddo
